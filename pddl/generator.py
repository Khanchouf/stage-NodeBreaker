...
def build_problem_text(problem: PlanningProblem) -> str:
    ...
    for cell in problem.cells:
        ...
        ].append(pddl_name(cell.id))

    init: list[str] = ["(= (total-cost) 0)", "(= (temporary-outages) 0)"]
    ...
    for node1, node2 in problem.internal_connections:
        ...
        init.extend([f"(internal-link {n1} {n2})", f"(internal-link {n2} {n1})"])
    for busbar in problem.busbars:
        init.append(f"(busbar-at {pddl_name(busbar.id)} {pddl_name(busbar.node)})")
    ...
    for cell in problem.cells:
        ...
        breaker_closed = any(initial.is_closed(problem, sid) for sid in cell.breaker_ids)
        ...
        if (
            ...
            and any(problem.equipment_by_id[eid].protected for eid in cell.equipment_ids)
        ):
        ...
        for disconnector_id in cell.disconnector_ids:
            ...
            for busbar in problem.busbars:
                if busbar.node in {switch.node1, switch.node2}:
                    init.append(
                        f"(disconnector-to-bar {pddl_name(disconnector_id)} {cid} {pddl_name(busbar.id)})"
                    )
    ...
    for switch in problem.switches:
        ...
        if len(adjacent) != 2:
            continue
        sid, b1, b2 = pddl_name(switch.id), pddl_name(adjacent[0]), pddl_name(adjacent[1])
        init.append(f"(couples {sid} {b1} {b2})")
        if initial.is_closed(problem, switch.id):
            init.extend([f"(bars-coupled {b1} {b2})", f"(bars-coupled {b2} {b1})"])

    goals = [
        _literal_closed(pddl_name(switch.id), target.is_closed(problem, switch.id))
        for switch in problem.switches
    ]
    object_lines = [
...
def export_pddl(problem: PlanningProblem, output_dir: str | Path) -> tuple[Path, Path]:
    ...
    shutil.copyfile(Path(__file__).with_name("domain.pddl"), domain_output)
